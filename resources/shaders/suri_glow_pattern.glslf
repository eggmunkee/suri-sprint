...
void main() {
    ...
    //vec4 pixColTL = texture(t_Texture, v_Uv + vec2(-10.0,-10.0)) * v_Color;
    //vec4 pixColBR = texture(t_Texture, v_Uv - vec2(10.0,10.0)) * v_Color;
    ...
    if (pixCol.a > 0.9) {
    ...
    else if ((pixCol.a > 0.5)) { //} && pixCol.a < 0.90)) { //  || tex_colors > 0.7
        float strength = 0.75-0.5*pixCol.a;
        ...
        float overlay_str = max(0.0, 1.0 - pow(2.5*cent_dist+ 0.1*cos(pos.x-pos.y) + 0.1*sin(-pos.x-pos.y), 2.0) - 0.1 * pixCol.a);
